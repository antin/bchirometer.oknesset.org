...
(function() {
  ...
  $.widget("mit.agendaSlider", $.extend({}, $.ui.slider.prototype, {
    _create: function() {
		...
		this.element.append('<div id="radio">');
		this.element.append('<input type="radio" id="rbtWith" name="radio" class="btn btn-success"><label for="rbtWith">(:</label>');
		this.element.append('<input type="radio" id="rbtNone" name="radio" class="btn btn-default" checked="checked"><label for="rbtNone">-</label>');
		this.element.append('<input type="radio" id="rbtAgainst" name="radio" class="btn btn-danger" ><label for="rbtAgainst">:(</label>');
...
//this.element.find(".btn-success").focus();
//alert(value);
	//alert("minus-" + value)
  ...
  multiSync = function(method, model, options) {
	...
	//if(responses)
	  //      return options.error.apply(options, responses[0]);
        return options.error.apply(options, null);
      }
    });
  };
  ...
  root.AgendaList = (function(_super) {
    ...
    AgendaList.prototype.url = "http://oknesset.org/api/v2/agenda/?extra_fields=num_followers,image,parties";
    ...
    root.CategoryList = (function(_super) {
    ...
    CategoryList.prototype.model = root.Category;

    CategoryList.prototype.multiSync = [
      {
        //url: '/data/categories.jsonp',
        repo: window.mit.categories,
        sync: root.JSONPCachableSync('categories')
      }
    ];

    CategoryList.prototype.sync = multiSync;
    /*
    CategoryList.prototype.initialize = function() {
      CategoryList.__super__.initialize.apply(this, arguments);
      return this.categories_fetching = $.Deferred().resolve();
    };
*/

    CategoryList.prototype.parse = function(response) {
        return response;
      };
  ...
  root.MemberList = (function(_super) {
    ...
    MemberList.prototype.multiSync = [
      {
        url: "http://oknesset.org/api/v2/member/?extra_fields=current_role_descriptions,party_name,links",
    ...
    MemberList.prototype.fetchAgendas = function() {
      ...
      bulkUrl = "http://oknesset.org/api/v2/member-agendas/set/" + ids.join(';') + '/';
...
*/
	NewbiesList.prototype.multiSync = [
      {
        url: "http://www.mitpakdim.co.il/site/primaries/candidates/json.php",
        repo: window.mit.combined_newbies,
        sync: root.JSONPCachableSync('combined_newbies_site')
      }
    ];

    NewbiesList.prototype.sync = multiSync;
  ...
  root.PartyDeclarationList = (function(_super) {
    ...
    PartyDeclarationList.prototype.multiSync = [
      {
        //url: '/data/party_declarations.jsonp',
        repo: window.mit.party_declarations,
        sync: root.JSONPCachableSync('party_declarations')
      }, {
  ...
  root.CandidateView = (function(_super) {
    ...
    CandidateView.prototype.events = {
      ...
      'click .mdhk': function(event) {
        ...
        window.location = 'madhak.html#' + id + ((weights != '') ? ('//' + weights):(''));
  ...
  root.DropdownContainer = (function(_super) {
    ...
    root.CategoryView = (function(_super) {

    __extends(CategoryView, _super);

    function CategoryView() {
      this.render = __bind(this.render, this);      return CategoryView.__super__.constructor.apply(this, arguments);
    }

    CategoryView.prototype.el = ".current_party";

    CategoryView.prototype.render = function() {
      if (root.global.party) {
        return this.$('.current_party_logo_back').html("<img class='current_party_logo' src='" + "'/>");
      }
    };

   CategoryView.prototype.get_template = function() {
      return $("#category_template").html();
    };
  ...
  setupPartyList = function() {
    ...
    //root.lists.categories = new root.CategoryList;
...
var data = $.getJSON("http://www.mitpakdim.co.il/site/primaries/data/categories.jsonp", function(data) { 
